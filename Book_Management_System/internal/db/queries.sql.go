...
const getSearchBooksCount = `-- name: GetSearchBooksCount :one
SELECT COUNT(*)
FROM books
...
func (q *Queries) GetSearchBooksCount(ctx context.Context, dollar_1 sql.NullString) (int64, error) {
	row := q.db.QueryRowContext(ctx, getSearchBooksCount, dollar_1)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
const searchBooksPaginated = `-- name: SearchBooksPaginated :many
SELECT book_id, author_id, publisher_id, title, publication_date, isbn, pages, genre, description, price, quantity
FROM books
WHERE LOWER(title) LIKE LOWER('%' || $1 || '%')
   OR LOWER(description) LIKE LOWER('%' || $1 || '%')
...
type SearchBooksPaginatedParams struct {
	Column1 sql.NullString
	Limit   int32
	Offset  int32
}

func (q *Queries) SearchBooksPaginated(ctx context.Context, arg SearchBooksPaginatedParams) ([]Book, error) {
	rows, err := q.db.QueryContext(ctx, searchBooksPaginated, arg.Column1, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
