...
export const useAuthors = (initialPage = 1, initialLimit = 10, fetchOnMount = true) => {
  ...
  const fetchAuthors = async (page = 1, limit = 10) => {
    ...
    try {
      ...
      let response;
      try {
        response = await authorService.getAll(page, limit);
      } catch (apiError) {
        console.error('Error fetching authors from API, using fallback:', apiError);
        response = {
          authors: [],
          total: 0,
          page,
  ...
  const addAuthor = async (authorData) => {
    ...
    try {
      ...
      try {
        ...
        newAuthor = await authorService.create(backendAuthorData);
        console.log('Author created via API:', newAuthor);
      } catch (apiError) {
        console.error('Error calling author API, using fallback:', apiError);
        ...
        console.log('Created author in fallback mode:', newAuthor);
      }
  ...
  const editAuthor = async (id, authorData) => {
    ...
    try {
      ...
      try {
        updatedAuthor = await authorService.update(id, authorData);
      } catch (apiError) {
        console.error('Error updating author via API, using fallback:', apiError);
        updatedAuthor = {
          ...authorData,
          authorId: id,
  ...
  const removeAuthor = async (id) => {
    ...
    try {
      try {
        ...
        console.error('Error deleting author via API, continuing with UI update:', apiError);
      }
